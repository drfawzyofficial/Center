...
router.post('/disagreement', ensureAuthenticated, async (req, res) => {
  ...
  // Post Request to /dashboard/upload
router.post('/upload', ensureAuthenticated, upload.single('avatar'), async (req, res) => {
    try {
       const { residentID } = req.body; 
      ...
      if(resident.accepted === false) {
         return res.status(422).json({
            statusCode: 422,
            error: 'يجب الموافقة على الطلب أولًا قبل رفع صورة',
         });
      }
      ...
      if (ext !== '.png' && ext !== '.jpg' && ext !== '.gif' && ext !== '.jpeg' && ext !== '.PNG' && ext !== '.JPG' && ext !== '.GIF' && ext !== '.JPEG') {
        return res.status(422).json({
            statusCode: 422,
            error: 'هذا النوع من الملف غير مدعوم',
         });
      }
